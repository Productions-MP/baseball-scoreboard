...
PORT="$(read_env_value "SCOREHLS_PORT")"
PORT="${PORT:-${DEFAULT_PORT}}"
DISPLAY_URL="$(read_env_value "SCOREHLS_DISPLAY_URL")"
...
if ! wait_for_http "${HEALTH_URL}" "${WAIT_ATTEMPTS}"; then
  echo "ScoreHLS web app did not become ready at ${HEALTH_URL} before the kiosk timeout." >&2
  exit 1
fi
